...
(function() {
	window.mcImageManager = {
		...
		openWin : function(f, a) {
			...
			// Use native dialogs
			w = window.open(f.url, 'mcImageManagerWin', 
								   'left=' + f.x + ',top=' + f.y + ',width=' + f.width + ',height=' + f.height + ',scrollbars=' + (f.scrollbars ? 'yes' : 'no') + ',resizable=' + (f.resizable ? 'yes' : 'no') + ',statusbar=no,location=no'
			);
