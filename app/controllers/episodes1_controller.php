...
class EpisodesController extends AppController {
	...
	var $paginate = array(
        'contain' => array('User', 'Reaction' => array('User')),
        'limit' => 10
    );
	...
	function fiche($show_menu, $season_episode) {
		...
		$episode = $this->Episode->find('first', array('conditions' => array('Season.show_id' =>  $show['Show']['id'], 'Season.name' => $no_saison, 'Episode.numero' => $no_episode)));
		...
		if(!empty($episode['Episode']['id'])) {
			...
			$comments = $this->Episode->Comment->find('all', array('conditions' => array('Comment.episode_id' => $episode['Episode']['id'], 'Comment.thumb' != '')), array('order' => array('Comment.id DESC'), 'limit' => 2, 'fields' => array('Comment.text', 'User.login', 'Comment.thumb', 'Show.name', 'Show.id')));
			...
			$critiques = $this->Article->find('all', array('conditions' => array('Article.category' =>  'critique', 'Article.episode_id' => $episode['Episode']['id'])));
