...
class PollsController extends AppController {
    ...
    function awards2016() {
        ...
        $polls = $this->Poll->find('first', array('contain' => array('Question' => array('Answer' => array('order' => 'Answer.porcent DESC'))), 'conditions' => array('id' => 5)));
        $this->set('polls', $polls);

        if ($this->Auth->user('role') > 0) {
            $votes = $this->Vote->find('all', array('conditions'=>array('User.id'=>$this->Auth->user('id'), 'Poll.id'=>5),'order'=>'Question.id ASC'));
            $this->set('votes', $votes);
        } else {
            $this->set('votes', '');
        }
