    ...
    <div id="col1">
    	<div class="padl10">
					...
					if ($continue and $compteur >= 0) {
						...
						if($jour_courant != $jour_prec) {
							...
							if ($compteur >= 0) {
								...
								echo' <span class="red">' . $html->link($episode['Season']['name'] . '.' . str_pad($episode['Episode']['numero'], 2, 0, STR_PAD_LEFT), '/episode/' . $episode['Season']['Show']['menu'] . '/s' . str_pad($episode['Season']['name'], 2, 0, STR_PAD_LEFT) . 'e' . str_pad($episode['Episode']['numero'], 2, 0, STR_PAD_LEFT), array('class' => 'decored')) . '</span>'; 
								echo ' - ';
           ...
           <div id="tabs-3">
				...
				// $priority2 = false;
				$compteur = 10;
				foreach ($mesepisodes as $episode) {
